		...
		switch (message.command) {
			...
			case "NICK":
				for (var chanName in self.chans) {
					var channel = self.chans[chanName.toUpperCase()];

					if ('string' == typeof channel.users[message.nick]) {
						messageToEmit.receiver = chanName.toUpperCase();
						...
						self.emit('change_nick', {
							channel: chanName.toUpperCase(),
			...
			case "JOIN":
				...
				messageToEmit.message = (self.nick == message.nick ? 'Now talking in ' + channel : message.nick + ' has joined');
			...
			case "PART":
				...
				messageToEmit.message = message.nick + ' has left';
			...
			case "KICK":
				messageToEmit.receiver = message.args[0];
				messageToEmit.message = '' + message.args[1] + ' has kicked ' + message.nick + ' (Reason: ' + message.args[2] + ')';
			...
			case "INVITE":
				messageToEmit.receiver = 'status';
				messageToEmit.message = '' + message.nick + ' is inviting you to join ' + message.args[1];
			...
			case "QUIT":
				...
				for (var chanKey in self.chans) {
					...
					if ('string' == typeof channel.users[message.nick]) {
						messageToEmit.receiver = chanName; //.toUpperCase();
						messageToEmit.message = message.nick + ' has quit (Message: ' + message.args[0] + ')';
