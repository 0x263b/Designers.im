...
io.sockets.on("connection", function(client) {
	...
	var IPV4 = new RegExp(/^(25[0-5]|2[0-4][0-9]|1?[0-9][0-9]{1,2})(\.(25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$/);
	var IPV6 = new RegExp(/^([0-9a-f]){1,4}(:([0-9a-f]){1,4}){7}$/i);
	var userName;

	var client_ip = client.request.headers["x-forwarded-for"] ? client.request.headers["x-forwarded-for"].split(',').pop() : client.request.connection.remoteAddress;
	if (IPV6.test(client_ip)) {
		userName = client_ip;
	} 
	if (IPV4.test(client_ip)) {
		var client_ip_blocks = client_ip.split(".");
		userName = "";
		client_ip_blocks.forEach(function(block){
			block = parseInt(block);
			userName += block.toString(16).toUpperCase();
		});
	}
	...
	client.on("connectToIRC", function(data) {
		...
		var opts = {
			...
			channels: ["#DN"],
			password: null,
			userName: userName || data.options.nickname,
			secure: true,
			selfSigned: true,
			certExpired: true,
			stripColors: true
